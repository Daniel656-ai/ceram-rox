...
export function useOrderDetail(orderId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("measurement_orders")
        .select(`*, projects(project_number, project_name), order_measurements(*, measurement_services(service_name, category, hourly_rate), measurement_parameters(*), work_logs(*), documents(*))`)
        .eq("id", orderId!)
        .single();
      if (error) throw error;
      return data;
