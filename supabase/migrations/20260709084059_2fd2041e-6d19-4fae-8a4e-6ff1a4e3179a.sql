...
AS $$
  WITH can_view AS (
    SELECT public.has_role(auth.uid(), 'master'::public.app_role)
      OR public.has_permission(auth.uid(), 'portfolios.view') AS allowed
  )
  SELECT
  ...
  WHERE cv.allowed
    AND pm.portfolio_id = _portfolio_id
  ...
  CROSS JOIN can_view cv
  WHERE cv.allowed
    AND ppm.portfolio_id = _portfolio_id
$$;
...
AS $$
  ...
  SELECT jsonb_build_object(
    ...
    'documents_total', (
      ...
      CROSS JOIN can_view cv
      WHERE cv.allowed
        AND d.portfolio_id = _portfolio_id
    ),
