...
BEGIN
  ...
  -- Felder kopieren (neue IDs, Mapping merken)
  FOR _old, _new IN
    WITH ins AS (
  ...
  -- Alle internen Referenzen (alte IDs) auf die neuen IDs umschreiben
  FOR _old, _new IN SELECT key, value #>> '{}' FROM jsonb_each(_map) LOOP
    ...
    UPDATE public.form_field_rules
      SET condition = replace(condition::text, _old, _new)::jsonb,
          target_field_ids = (
            SELECT array_agg(CASE WHEN t = _old THEN _new ELSE t END ORDER BY ord)
  ...
  -- Verschachtelte Felder: parent_field_id auf die Kopien umbiegen
...
CREATE OR REPLACE FUNCTION public.rename_form_definition(_form_id uuid, _new_name text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  IF NOT public.can_manage_designer(auth.uid()) THEN
    RAISE EXCEPTION 'Keine Berechtigung zum Umbenennen von Formularen';
  END IF;
  ...
  SELECT * INTO _src FROM public.form_definitions WHERE id = _form_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Formular nicht gefunden';
  END IF;
  ...
  IF EXISTS (
    SELECT 1 FROM public.form_definitions
    WHERE id <> _form_id AND scope = _src.scope AND archived_at IS NULL
      AND lower(btrim(name)) = lower(_name)
  ) THEN
    RAISE EXCEPTION 'Ein Formular mit der Bezeichnung "%" existiert bereits', _name;
  END IF;

  UPDATE public.form_definitions SET name = _name, updated_at = now() WHERE id = _form_id;
END;
$$;

REVOKE EXECUTE ON FUNCTION public.rename_form_definition(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rename_form_definition(uuid, text) TO authenticated, service_role;