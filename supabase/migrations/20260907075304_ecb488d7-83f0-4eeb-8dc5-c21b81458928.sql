CREATE OR REPLACE FUNCTION public._clone_form_full(_source_form_id uuid, _new_name text DEFAULT NULL, _new_scope text DEFAULT NULL)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  VALUES (COALESCE(NULLIF(btrim(coalesce(_new_name,'')),''), _src.name), _src.description,
          COALESCE(_new_scope::form_scope, _src.scope), 1, _src.layout, auth.uid())
  ...
  -- Feldregeln (optionale Struktur)
  IF to_regclass('public.form_field_rules') IS NOT NULL THEN
  ...
  ELSE
    RAISE WARNING 'Kopieren: Tabelle public.form_field_rules fehlt – Feldregeln wurden nicht kopiert (Formular %)', _source_form_id;
  END IF;
  ...
  -- Referenzen auf die neuen IDs umschreiben
...
EXCEPTION
  WHEN undefined_table OR undefined_column THEN
    RAISE EXCEPTION 'Kopieren fehlgeschlagen: eine benötigte Datenstruktur fehlt (%). Bitte ausstehende Datenbankmigrationen prüfen.', SQLERRM
...
BEGIN
  ...
  RETURN public._clone_form_full(_source_form_id, _name, NULL);
END;
$$;

CREATE OR REPLACE FUNCTION public._clone_form_definition(_source_form_id uuid, _new_scope text DEFAULT NULL::text)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.form_definitions WHERE id = _source_form_id) THEN
    RETURN NULL;
  END IF;
  RETURN public._clone_form_full(_source_form_id, NULL, _new_scope);
END;
$$;

REVOKE ALL ON FUNCTION public._clone_form_full(uuid, text, text) FROM PUBLIC;
