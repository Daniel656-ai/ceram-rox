...
/* ---------------------------------------------------------------------------
 * Hoch-/Tiefstellung (siehe src/lib/richText.ts)
 * Die Auszeichnung `_{...}` / `^{...}` wird für die PDF-Ausgabe in Unicode-
 * Sub/Superscript umgesetzt. Zeichen ohne Unicode-Äquivalent bleiben erhalten,
 * unformatierter Text bleibt unverändert (abwärtskompatibel).
 * ------------------------------------------------------------------------- */
const RT_SUB: Record<string, string> = {
  "0":"₀","1":"₁","2":"₂","3":"₃","4":"₄","5":"₅","6":"₆","7":"₇","8":"₈","9":"₉",
  "+":"₊","-":"₋","−":"₋","=":"₌","(":"₍",")":"₎",
  a:"ₐ",e:"ₑ",h:"ₕ",i:"ᵢ",j:"ⱼ",k:"ₖ",l:"ₗ",m:"ₘ",n:"ₙ",o:"ₒ",p:"ₚ",r:"ᵣ",s:"ₛ",t:"ₜ",u:"ᵤ",v:"ᵥ",x:"ₓ",
};
const RT_SUP: Record<string, string> = {
  "0":"⁰","1":"¹","2":"²","3":"³","4":"⁴","5":"⁵","6":"⁶","7":"⁷","8":"⁸","9":"⁹",
  "+":"⁺","-":"⁻","−":"⁻","=":"⁼","(":"⁽",")":"⁾",
  a:"ᵃ",b:"ᵇ",c:"ᶜ",d:"ᵈ",e:"ᵉ",f:"ᶠ",g:"ᵍ",h:"ʰ",i:"ⁱ",j:"ʲ",k:"ᵏ",l:"ˡ",m:"ᵐ",n:"ⁿ",o:"ᵒ",
  p:"ᵖ",r:"ʳ",s:"ˢ",t:"ᵗ",u:"ᵘ",v:"ᵛ",w:"ʷ",x:"ˣ",y:"ʸ",z:"ᶻ",
};
function richToUnicode(input: unknown): any {
  if (typeof input !== "string" || !/[_^]\{/.test(input)) return input;
  return input.replace(/([_^])\{([^}]*)\}/g, (_m, sign: string, inner: string) => {
    const map = sign === "_" ? RT_SUB : RT_SUP;
    return [...inner].map((c) => map[c] ?? c).join("");
  });
}
/** Zentrale Textausgabe: jede PDF-Zeile läuft durch die Formatierungslogik. */
function withRichText(doc: any) {
  const orig = doc.text.bind(doc);
  doc.text = (t: any, ...rest: any[]) =>
    orig(Array.isArray(t) ? t.map(richToUnicode) : richToUnicode(t), ...rest);
  const origSplit = doc.splitTextToSize.bind(doc);
  doc.splitTextToSize = (t: any, ...rest: any[]) => origSplit(richToUnicode(t), ...rest);
  return doc;
}
...
function renderPdf(snapshot: any, layout: any) {
  const doc = withRichText(new jsPDF({ unit: "pt", format: "a4" }));
...
function renderBlockTemplate(snapshot: any, tpl: any) {
  ...
  const doc = withRichText(new jsPDF({ unit: "pt", format: "a4", orientation }));
  applyUnicodeFont(doc);
  const pageWidth = doc.internal.pageSize.getWidth();
  const pageHeight = doc.internal.pageSize.getHeight();
  const marginX = 40;
